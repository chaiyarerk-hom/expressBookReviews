...
public_users.get('/', (req, res) => {
  ...
  .catch(error => { return res.status(404).json({message: "Error fetching data:", error: error.message}); });
});
...
public_users.get('/author/:author',function (req, res) {
  ...
  axios.get('http://localhost:5000/books' + author)
  .then(response => { return res.status(200).json(response.data) })
  .catch(error => { return res.status(404).json({message: "Error fetching data:", error: error.message}); });
});
...
axios.get('http://localhost:5000/books' + title)
  .then(response => { return res.status(200).json(response.data) })
  .catch(error => { return res.status(404).json({message: "Error fetching data:", error: error.message}); });
});
